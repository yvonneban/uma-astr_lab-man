...
General information about the setup of the course can be found in Section~\ref{sec:setup}, including grade breakdown and rubrics. The prep, procedure, and script for each lab is found in Section~\ref{sec:lab}.

Labs are organised by content, and the order presented here does not prescribe or suggest a schedule. The exact schedule for each semester is at the discretion of the organising instructor. I've moved the material covering the lab projects into their independent sections for organisation.

\subsection{Prep and Script}\label{sec:howto_prep}

I have designed it such that most of the script for each class can be essentially read off the slides. More details can be found in the accompanying script.

The pre-lab survey and lab quizzes are also included for reference.

\subsection{Worksheets and Handouts}\label{sec:howto_ws}

\textbf{NOTE: THE WORKSHEETS IN THIS MANUAL ARE \emph{NOT} PRINT READY!!} To compile print-ready worksheets, reference Section \ref{sec:howto_tex} and use the worksheet master document \texttt{ws.tex}.

The content of the worksheets and handouts can be found in the appendices for reference.

\subsection{Using \texttt{.tex} files}\label{sec:howto_tex}

Each worksheet is in its independent \texttt{.tex} file so they can be compiled in the manual (this file, \texttt{main.tex}) or for printing in \texttt{ws.tex}. Therefore, to edit the content of a worksheet, use the specific \texttt{.tex} file for the worksheet.

To use \texttt{ws.tex} to compile worksheets \textbf{for printing}, simply open the \texttt{.tex} file and arrange the code blocks for each respective lab in the scheduled order. To skip labs that are not part of the schedule, just comment the code block out. This should compile a master worksheet document with all of the worksheets in order and properly paginated for printing.

\textbf{Take note of labs with table worksheets:} Labs \ref{apx:lab_03} (Angular Size vs. Distance), \ref{apx:lab_06} (Solar Power), \ref{apx:lab_09} (The HR Diagram), and \ref{apx:lab_10} (Distances of Stars in Space) have table worksheets. \textbf{Don't forget the table worksheet code block when formatting!} I've also tagged the code blocks in \texttt{ws.tex}.

Lastly, beware of breaking paths if you reorganise files, particularly with moving the images in the directory. I probably should have moved them all into an img directory for neatness, but I didn't, and now we all have to live with it, as is the way with history.
...
\begin{itemize}
    \item the Sun is so much brighter that we canâ€™t look at it directly.
...
\end{itemize}
\item
Which of the following is a significant cause of Amherstâ€™s summers being warmer than its winters?
\begin{itemize}
...
\end{enumerate}
Which way did you turn your head? (Clockwise or counterclockwise, looking down on your head from above the Ònorth poleÓ?)
& \includegraphics[width=0.5\textwidth]{\slidepath{4}}\\\hline
\begin{enumerate}
\item Put the golf ball on a pencil/pen so you can hold it up. This will represent the Moon. 
\item The Moon orbits the Earth counterclockwise as seen from above the north pole (and similar for the Earth and Sun)Ñthe same way as you turned your head. Slowly turn in that direction with the Moon held out and look at how the Sun lights it up as you turn.
\end{enumerate}
& \includegraphics[width=0.5\textwidth]{\slidepath{5}}\\\hline
...
\begin{enumerate}
\item Set the location to Amherst Center and the date to 28 Sept 2021. Turn on the meridian line (either in the Sky and Viewing Window or by hitting Ô;Õ). 
...
\item Open the Sky and Viewing Options window.
	...
	\end{enumerate}
\end{enumerate}
For the web version: Turn on meridian line under ÒView SettingsÓ and set location by clicking the lower-left grey box.)
& \includegraphics[width=0.5\textwidth]{\slidepath{3}}\\\hline
...
\begin{itemize}
\item Make sure everyone participates in observation \textbf{and} does calculation at some point in experiment 1 or 2. If you have a large group, you can repeat and check each otherÕs calculations.
...
At our latitude at the summer solstice, the SunÕs light is spread over an area almost as small as at the Tropic of Cancer, so the intensity is high and the ground and atmosphere heat most.
& \includegraphics[width=0.5\textwidth]{\slidepath{8}}\\\hline
Solstices: Winter

The Sun shines straight down on the Tropic of Capricorn, \SI{23.5}{\degree} South of the equator.

At our latitude at the winter solstice, the SunÕs light is spread over a much larger area, so the intensity is much lower and the ground and atmosphere cool down.

We're actually a few million kilometres closer to the Sun in January, but thatÕs a proportionately tiny change in distance and not as important!
& \includegraphics[width=0.5\textwidth]{\slidepath{9}}\\\hline
\emph{At $\sim$10 min from class end, show Moodle quiz password.} 
...
\end{enumerate}
\item
In part 3, what distance did your group estimate had the same intensity as light shining from an angle of 24Â°? [\underline{1.5}] ft (enter number only)
\item
...
\newpage
\subsection{Lab 7: Calendars, Horoscopes, and Precession}\label{sec:lab_07}
...
First fully remote semester. Adapted labs to be 2 weeks each with lecture in first week, remote and recorded. Lab 2: Angular Sizes on the Sky (\ref{sec:lab_02}) and camera calibration for project were skipped due to remote learning. Extra-credit project (\ref{sec:exproj}) was used for credit.

\begin{table}[htbp]
\caption{Fall 2020 schedule}
\begin{center}
\begin{tabular}{|c|l|}\hline
\textbf{Week} & \textbf{Lab}\\\hline
1 & Lab 1: Astronomical Angles and Stellarium (\ref{sec:lab_01})\\\hline
3 & Lab 5: Motions of the Sun (\ref{sec:lab_05}) and Project (\ref{sec:proj})\\\hline
5 & Lab 4: Phases of the Moon (\ref{sec:lab_04}) and Extra-credit project (\ref{sec:exproj})\\\hline
7 & Lab 7: Calendars, Horoscopes, and Precession (\ref{sec:lab_07})\\\hline
9 & Lab 8: Motions of the Planets in the Sky (\ref{sec:lab_08})\\\hline
11 & Lab 9: The HR Diagram (\ref{sec:lab_09})\\\hline
13 & Turn in Project\\\hline
\end{tabular}
\end{center}
\label{tab:f20}
\end{table}
...
Fully remote semester. Same format as \ref{sec:past_f20}.

\begin{table}[htbp]
\caption{Spring 2021 schedule}
\begin{center}
\begin{tabular}{|c|l|}\hline
\textbf{Week} & \textbf{Lab}\\\hline
1 & Lab 1: Astronomical Angles and Stellarium (\ref{sec:lab_01})\\\hline
...
\begin{table}[htbp]
\caption{Fall 2021 schedule}
\begin{center}
\begin{tabular}{|c|l|}\hline
\textbf{Week} & \textbf{Lab}\\\hline
1 & Lab 1: Astronomical Angles and Stellarium (\ref{sec:lab_01})\\\hline
2 & Lab 1: Astronomical Angles and Stellarium (\ref{sec:lab_01})\\\hline
3 & Lab 5: Motions of the Sun (\ref{sec:lab_05})\\\hline
4 & Lab 2: Angular Sizes on the Sky (\ref{sec:lab_02}) and Project (\ref{sec:proj})\\\hline
5 & Lab 4: Phases of the Moon (\ref{sec:lab_04}) and Extra-credit project (\ref{sec:exproj})\\\hline
6 & Lab 6: Solar Power (\ref{sec:lab_06})\\\hline
7 & Makeups \\\hline
8 & Lab 3: Angular Size vs. Distance (\ref{sec:lab_03})\\\hline
9 & Lab 9: The HR Diagram (\ref{sec:lab_09})\\\hline
10 & Lab 10: Distances of Stars in Space (\ref{sec:lab_10})\\\hline
11 & Lab 11: Structure of the Local Group (\ref{sec:lab_11})\\\hline
12 & Turn in Project/makeups \\\hline
\end{tabular}
\end{center}
\label{tab:f21}
\end{table}
...
First, foremost, and most fundamental, our deepest and highest thanks go to Prof. Stephen Schneider, a veritable fount of both knowledge and wisdom throughout his leadership of the course and and his guidance of us, without whom this course could not possibly accomplish even the most basic understanding and organisation, much less the transformative learning and enlightenment that it does. Needless to say, without such unfailingly steadfast and tireless efforts, this manual, which only aims to faithfully capture the amazing work already put into composing and refining this unique course, could never even observe the electromagnetic radiation of solar daytime. In particular, with the onset of the COVID-19 pandemic and accompanying worldwide quarantine measures, Prof. Schneider pulled from unfathomable reserves of persistence, foresight, and knowledge to adapt the course on-the-fly for the abrupt and drastic challenges that befell us, a feat that is both impressive and inspirational. I am enormously grateful for everything he has taught me about teaching, life, the universe, and everything, and I know his students are similarly grateful for his utmost dedication to their learning.

In a close second place, I must thank (the now Dr.!) Patrick Kamieneski, for his many years leading the on-the-ground running of the course. I was able to draw extensively on his example and his knowledge to make this manual the most complete and comprehensive it could be, and indeed his efforts were essential, both in the continued smooth running of the course through times both thick and thin, and in updating and adapting the course for each semester. I am especially grateful to him both for his memory and for letting me tap out of leading lab sections without complaint. I wish him the best and look forward to his continued achievement in his upcoming career.

I must of course extend my deepest gratitude to my other fellow T.A.s in the trenches, those of us who shared in the noble work of education together. I do wish to single out Sumner Gubisch for generously helping me with the tedious busywork of this document, and, along with all our other undergrad T.A.s, sparing his time and energy to help with making our classes run as smoothly as possible. The maturity and leadership that they consistently demonstrate gives me great assurance for their future endeavours, and it was a pleasure working with each and every one of them. I'd also like to thank Sarah Linehan for her contributions to this manual, and apologise to Sarah Bodansky for spilling my entire bottle of hot tea over both our computers in the middle of class. To my other fellow T.A.s, my words fail me and my gratitude must be encapsulated in a humble: thank you.

Last but, of course, not least, my thanks to my students, especially those who gave me feedback about the course. After all, they are who I made this for.

\newpage
\begin{appendices}

N.B.: The worksheets here are for reference only and are \textbf{not} print-ready. To get print-ready worksheets, reference Section \ref{sec:howto_tex}, and use the worksheet master document \texttt{ws.tex}.
